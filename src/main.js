(function(){
  ...
  const TILE_W=64, TILE_H=32, CHUNK_TILES=48, LOAD_RADIUS=1, BIOME_SCALE=80;
  ...
  function valueNoise2D(ix,iy,scale){ const x=ix/scale,y=iy/scale; const x0=Math.floor(x),y0=Math.floor(y),x1=x0+1,y1=y0+1;
    const sx=x-x0, sy=y-y0; const lerp=(a,b,t)=>a+(b-a)*t; const n=(i,j)=>seedRand((i*374761393)^(j*668265263));
    const n00=n(x0,y0), n10=n(x1,y0), n01=n(x0,y1), n11=n(x1,y1); const ix0=lerp(n00,n10,sx), ix1=lerp(n01,n11,sx); return lerp(ix0,ix1,sy); }
  ...
  function ensureChunk(cx,cy){
    ...
    for(let li=0; li<CHUNK_TILES; li++) for(let lj=0; lj<CHUNK_TILES; lj++){
      ...
      const biome=valueNoise2D(wi,wj,BIOME_SCALE), dens= biome>0.55 ? TREE_DENS_WALD : TREE_DENS_WIESE;
      const r=valueNoise2D(wi*13+7,wj*7+11,8); if(r<dens){ trees.set(wi+','+wj,{i:wi,j:wj}); }
    }
  ...
  function unloadFarChunks(){
    ...
    for(const [key,ch] of state.chunks){ const dx=Math.abs(ch.cx-cx), dy=Math.abs(ch.cy-cy); if(dx>LOAD_RADIUS || dy>LOAD_RADIUS) state.chunks.delete(key); }
    $('chunks').textContent=state.chunks.size;
  }
  function loadAroundPlayer(){
    const {cx,cy}=worldToChunk(state.player.i,state.player.j);
    for(let x=cx-LOAD_RADIUS;x<=cx+LOAD_RADIUS;x++) for(let y=cy-LOAD_RADIUS;y<=cy+LOAD_RADIUS;y++) ensureChunk(x,y);
    unloadFarChunks();
  }
